		...
		<scroll-view scroll-y class="page">
			...
			<view class="bg-img padding-bottom-xl motion_top animation-fade" v-if="goalC[0]">
				...
				<view class="m-time heart1">
					<view class="flex">
						<view class="cu-tag cu-tag-top round line-blue shadow"><text
								class="text-blue">{{ goalC[0].progress }}</text></view>
				...
				<view class="m-kll heart2">
					<view class="flex ">
						<view class="cu-tag cu-tag-top round line-cyan shadow"><text
								class="text-cyan">{{ goalC[2].progress }}</text></view>
						<view class="flex-sub page-m">
				...
				<view class="m-foot heart3">
					<view class="flex">
						<view class="cu-tag cu-tag-top round line-red shadow"><text
								class="text-red">{{ goalC[1].progress }}</text></view>
						<view class="flex-sub page-m">
			...
			<scroll-view scroll-x="true" class="page_motion_tag animation-fade" v-if="ydList[0]">
				...
				<view class="flex">
					<view class="flex-sub" v-for="(item,index) in ydList" :key="index">
						<view class="cu-card article" @tap="ydToUrl(index)">
							<!--跳转到详情-->
							<view class="cu-item bar-shadown bg-img bg-img-yd"
						...
						<view class="cardTitle-jk">
							<view class="flex">
								<!-- <scroll-view scroll-x class="bg-white nav text-center">
									<view class="cu-item" :class="index==TabCur?'text-blue jk-cur cur':''"
										v-for="(jk,index) in jkList" :key="index" @tap="tabSelect" :data-id="index">
										<view v-if="index=='sleep'">睡眠</view>
										<view v-if="index=='heartbeat'">心率</view>
										<view v-if="index=='pulse'">脉搏</view>
									</view>
								</scroll-view> -->
								<!-- <view v-for="(value, key, index) in jkList"> -->
								<scroll-view scroll-x class="bg-white nav text-center">
									<view v-for="(jk,index) in jkList" :key="index">
										<view class="cu-item" :class="name==TabCur?'text-blue jk-cur cur':''"
											v-for="(value, name, index) in jk" :key="name" @tap="tabSelect"
											:data-id="name">
											<view v-if="name=='sleep'">睡眠</view>
											<view v-if="name=='heartbeat'">心率</view>
											<view v-if="name=='pulse'">脉搏</view>
										</view>
									</view>
								</scroll-view>
							</view>
							...
							<view v-if="TabCur=='sleep'">
								<view class="flex animation-fade jk-text"
									v-for="(sleepD,index) in jkList[0].sleep[0].content" :key="index">
									<view class="cu-timeline">
										...
										<view class="cu-item" :style="[{color:sleepD.color}]" v-if="sleepD.content">
											<view class="content"
												:style="[{ backgroundColor:sleepD.color,color:'#FFFFFF'}]">
												{{sleepD.content}}
											</view>
										</view>
									</view>
				...
				if (jkData != "") {
					...
					getJkList()
						.then(res => {
							...
							// console.log(_this.jkList)
							// console.log('-----')
							// console.log(_this.jkList[0].sleep[0].content)
